...
def updateTeams(a, b):
    dict_a= {a[i]: a[i + 1] for i in range(0, len(a), 2)}
    dict_b= {b[i]: b[i + 1] for i in range(0, len(b), 2)}
    #if there is duplicate kind in a, remove one with lowest strength
    for key, value in dict_a.items():
        #remove duplicate key with lowest value
        if key in dict_a:
            if value < a[key]:
                dict_a.pop(key)
    #if there is duplicate kind in b, remove one with lowest strength
    for key, value in dict_b.items():
        #remove duplicate key with lowest value
        if key in dict_b:
            if value < dict_b[key]:
    ...
    #if length dict_a greater that dict_b, remove first key in dict_a
    if len(dict_a) > len(dict_b):
        dict_a.popitem(last=False)
    #if length dict_b greater that dict_a, remove first key in dict_b
    if len(dict_b) > len(dict_a):
        dict_b.popitem(last=False)
        #convert dicts back to list
    dict_a = list(dict_a.keys())
    dict_b = list(dict_b.keys())
    return dict_a, dict_b
